	...
	return (
		<div
			className={`flex flex-col bg-white shadow-md my-16 mx-10 p-6 rounded ${
				featured && 'border-l-4 border-purple-500 border-solid'
			} sm:flex-row sm:my-4`}
		>
			<div>
				<img
			...
			<div className='flex flex-col justify-between ml-4'>
				<h3 className='font-bold text-purple-500'>
					{company}
					{isNew && (
						<span className='bg-red-500 font-bold text-white m-2 py-1 px-2 rounded-full text-sm uppercase'>
							New
						</span>
					)}
					{featured && (
						<span className='bg-gray-700 font-bold text-white m-2 py-1 px-2 rounded-full text-sm uppercase'>
							Featured
						</span>
					)}
				</h3>
